...
import { X, Moon, Sun, Mail, Bell, RefreshCw, Type, Layout, Image } from 'lucide-react';
import { Category } from '../types/article';
import { Switch } from '@/components/ui/switch';

export interface AppSettings {
  darkMode: boolean;
  emailDeliveryMode: boolean;
...
export function loadSettings(): AppSettings {
  return {
    ...
    emailDeliveryMode: localStorage.getItem('emailDeliveryMode') === 'true',
    notificationSound: localStorage.getItem('notificationSound') !== 'false',
    refreshInterval: (Number(localStorage.getItem('refreshInterval')) || 60) as 30 | 60 | 120,
    ...
    visibleCategories:
      JSON.parse(localStorage.getItem('visibleCategories') || 'null') || ALL_CATEGORIES,
...
export function saveSettings(settings: AppSettings) {
  ...
  localStorage.setItem('emailDeliveryMode', String(settings.emailDeliveryMode));
  ...
  localStorage.setItem('visibleCategories', JSON.stringify(settings.visibleCategories));
  localStorage.setItem('userName', settings.userName);
  localStorage.setItem('userEmail', settings.userEmail);
}

export default function SettingsPanel({ isOpen, settings: initialSettings, onClose, onSave }: SettingsPanelProps) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="settings-panel-enter fixed right-0 top-0 bottom-0 z-50 w-full max-w-sm flex flex-col"
        style={{
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  {local.darkMode ? <Moon className="w-4 h-4 text-muted-foreground" /> : <Sun className="w-4 h-4 text-muted-foreground" />}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm text-foreground">Email Delivery Mode</p>
                    <p className="text-xs text-muted-foreground">Disaster alerts via email</p>
                  </div>
                </div>
                <Switch
                  checked={local.emailDeliveryMode}
                  onCheckedChange={(v) => update('emailDeliveryMode', v)}
                />
              </div>
              ...
              <div className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-foreground">Notification Sound</span>
                </div>
                <Switch
            ...
            <div className="grid grid-cols-2 gap-2">
              {ALL_CATEGORIES.map((cat) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${isVisible ? 'bg-current' : 'bg-white/20'}`} />
                    {CATEGORY_LABELS[cat]}
                  </button>
                );
              })}
