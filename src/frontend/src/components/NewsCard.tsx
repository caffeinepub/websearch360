...
export default function NewsCard({
  ...
  return (
    <article
      className={`news-card card-hover glass rounded-xl border border-white/8 overflow-hidden flex flex-col group ${compact ? "p-3" : "p-0"}`}
    >
      {/* Thumbnail */}
      {showImages && !compact && (
        <div className="news-card-image relative w-full aspect-video overflow-hidden bg-gradient-to-br from-white/5 to-white/2">
          <img
            src={
              article.imageUrl ||
              "/assets/generated/news-thumbnail-placeholder.dim_400x225.png"
            }
          ...
          <div className="absolute top-2 left-2">
            <span
              className={`badge-${article.category} inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold border`}
            >
      ...
      <div className={`flex flex-col flex-1 ${compact ? "" : "p-4"}`}>
        ...
        {/* Headline */}
        <h3
          className={`font-display font-bold text-foreground leading-snug mb-2 ${headlineSizeClass} group-hover:text-cyan-400 transition-colors`}
        >
        ...
        {!compact && (
          <p
            className={`news-card-summary text-muted-foreground leading-relaxed mb-3 flex-1 line-clamp-3 ${fontSizeClass}`}
          >
        ...
        {/* Footer */}
        <div
          className={`flex items-center justify-between gap-2 mt-auto ${compact ? "mt-2" : ""}`}
        >
          <div className="flex items-center gap-2 min-w-0">
            <span
              className={`font-semibold text-foreground truncate ${fontSizeClass}`}
            >
              {article.source}
            </span>
