...
export default function NewsTicker({ headlines }: NewsTickerProps) {
  const doubled = headlines
    .map((h, i) => ({ text: h, key: `a-${i}` }))
    .concat(headlines.map((h, i) => ({ text: h, key: `b-${i}` })));
        ...
        <div className="flex-1 overflow-hidden relative">
          <div className="ticker-track">
            {doubled.map((item) => (
              <span
                key={item.key}
                className="inline-flex items-center text-white text-xs font-medium whitespace-nowrap px-6"
              >
